...
const eventFormSchema = z.object({
  ...
  date: z.string().refine((val) => !isNaN(Date.parse(val)), { message: 'A valid date is required' })
    .refine((val) => new Date(val) >= new Date(new Date().setHours(0,0,0,0)), { message: 'Date must be today or in the future' }),
...
export default function CreateEventPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const res = await api.createEvent(result.data);
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-12 text-white font-sans">
      ...
      <div className="rounded-lg border border-navy-border bg-navy-card p-8 shadow-sm">
        <h2 className="text-2xl font-bold tracking-tight text-white mb-6 font-sora">Create New Event</h2>
